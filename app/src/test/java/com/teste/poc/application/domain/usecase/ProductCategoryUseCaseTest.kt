package com.teste.poc.application.domain.usecase
...
import com.teste.poc.commons.CoroutinesTestRule
...
class ProductCategoryUseCaseTest {
    ...
    fun `when execute use case return mock success`() {
        coEvery { repo.getProducts(ZERO) } returns Product.mock()

        val response = runBlocking { useCase.execute(ZERO) }

        Assertions.assertThat(response).isEqualTo(Product.mock())
        coVerify(exactly = 1) { repo.getProducts(any()) }
    }

    @Test
    fun `when execute use case return mock error`() {
        coEvery { repo.getProducts(ZERO) } throws Exception(MOCK_EXCEPTION)
